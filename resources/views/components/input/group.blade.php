...
@if ($inline)
    <div>
        <label for="{{ $for }}" class="block text-sm font-medium leading-5 text-gray-700">{{ $label }} {!! $required ? '<span class="inline-block text-red-600 ml-0.5">*</span>' : '' !!}</label>

        <div class="mt-1 relative rounded-md shadow-sm {{ $width }}">
            {{ $slot }}
        </div>
