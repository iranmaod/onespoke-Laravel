<div x-data="{ open: @entangle('open') }">
    ...
    <x-admin.off-canvas>
        <div>
            <h3 class="text-2xl font-semibold text-gray-900">{{ empty($type) || !$type->exists ? 'Add New Type' : 'Edit Type: ' . $type->name }}</h3>
            ...
            <form wire:submit.prevent="save">
                ...
                <div class="mt-6 sm:mt-5">
                    ...
                    <x-input.group label="Category" for="categoryId" :error="$errors->first('categoryId')">
                        <x-input.select wire:model.defer="categoryId" id="categoryId">
                            <option value="">Select Category</option>
                            @foreach ($categories as $category) {
                                <option value="{{ $category->id }}" {{ !empty($type) && $type->category_id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                            @endforeach
                        </x-input.select>
                    </x-input.group>
                    ...
                    >
                        ...
                        <div class="px-2 mb-4 flex justify-between items-center">
                            ...
                            <div class="flex-1 pr-2">
                                <div class="relative">
                                    <x-input.text wire:model.debounce.500ms="search" id="search" placeholder="Name or Category" />
                                </div>
                            </div>
