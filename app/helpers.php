...
function gateway_payment($settings, $user, $total_supplier_price, $cart, $payment_gateway)
{
    ...
    foreach ($cart->content() as $product) {
        ...
        $pro = Products::where('id', $product->id)->first();
    ...
    Mail::send('app.emails.invoice_approved', $data, function ($message) use ($data, $settings) {
        $message->from($settings->site_email, $settings->site_name);
        $message->to("talhanadeem1721@gmail.com"); //sends to user
        $message->subject($data['subject']);
        $message->bcc("talhanadeem1721@gmail.com", 'Admin'); //sends to admin
        // $message->reply_to();
        // $message->cc();
    });
    Session::flash('info',  __('Prepping Product(s) for Shipment'));
    Session::flash('success', 'Thank You!. Payment was successful.');
    // alert()->success('Success', __('Thank You!. Payment was Successful.'))->showCloseButton()->autoClose(15000);
}
